...
$update_result=true;
        ...
        $bank=$_GET['bank'];
            if(isset($_POST['submit'])){
                ...
                $date=date("Y-m-d");
                $payment="INSERT INTO `payment_other_bank` (account_number,status,amount,bank,date,user_id)  VALUES ('{$to_acc}','{$msg1}','{$amount}','{$bank}','{$date}','{$_SESSION['user_id']}')";
                ...
                if($update_result){
                    ...
                    if(!$mail->send()) {
                      ...
                      header("location:other_bank_tranfer_succuss_page.php?to_acc=$to_acc&amount=$amount&bank=$bank&email=$email&s_acc=$acc_n&acc_name=$acc_name&s_acc=$s_acc&msg=$msg");
                }else{
                    $msg="Error";
                } 
        }
